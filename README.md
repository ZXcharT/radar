...
私有计算逻辑、数据流水线、调度脚本和生产运维配置不在本仓库维护。本仓库不是产品开发事实源；页面、交互和数据修改均在私有 `OneChart` 完成并通过发布链生成。

## 镜像验收

需要检查已生成的公开产物时，在仓库根目录运行：
...
- 本仓库只接收私有 `OneChart` 生成并验证的完整公开产物，不直接修改页面、样式、交互、数据或生成资产；
- 数据文件、manifest、指纹文件名和页面引用必须作为同一批发布产物保持一致；
- 只有 kg 明确批准的镜像治理元数据可以直接维护；
- 不提交私有算法、原始数据、凭据、内部路径、日志或运行轨迹；
- commit、push、线上发布、域名和 DNS 修改需要分别获得相应批准。

操作本仓库前请先阅读 [`AGENTS.md`](AGENTS.md)。
